...
def accuracy(sess, dataset, batch_size, X, Y, accuracy_op, mnist):
...
def train(sess, mnist, n_training_epochs, batch_size,
          ...
          summaries_op = None, accuracy_summary_op = None, 
          train_writer = None, test_writer = None):
    # compute number of batches for given batch_size
...
def runing(config, budget):
    ...
    with tf.Session() as sess:
        ...
        with open("results.csv", "a") as f:
            f.write("{0},{1},{2},{3}\n".format(sid, network, train_accuracy, train_accuracy))
